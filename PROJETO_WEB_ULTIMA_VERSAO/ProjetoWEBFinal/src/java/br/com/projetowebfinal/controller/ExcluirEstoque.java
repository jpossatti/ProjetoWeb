...
public class ExcluirEstoque extends HttpServlet {
    ...
    throws ServletException, IOException {
               Integer id_estoque = Integer.parseInt(request.getParameter("id_estoque"));
        try{
            EstoqueDAO dao= new EstoqueDAOImp();
            dao.excluirEstoque(id_estoque);
            request.getRequestDispatcher("ListarEstoque").forward(request, response);

        }catch (Exception ex){
            System.out.println("Problemas no Servlet as excluir estoque !Erro:"+ex.getMessage());
            ex.printStackTrace();
        }
    }
