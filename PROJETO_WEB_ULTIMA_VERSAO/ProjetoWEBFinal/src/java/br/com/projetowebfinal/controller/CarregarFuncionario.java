    ...
    throws ServletException, IOException {
       int id_pessoa = Integer.parseInt(request.getParameter("id_pessoa"));
        try{
            ...
            request.setAttribute("funcionario", dao.listarFuncionario(id_pessoa));
            request.setAttribute("cargos", daoC.listar());
            request.getRequestDispatcher("alterarfuncionario.jsp").forward(request, response);
        } catch(Exception ex){
            System.out.println("problemas no servlet ao carregar funcionario!:"+ex.getMessage());
            ex.printStackTrace();
