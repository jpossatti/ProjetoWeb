    ...
    throws ServletException, IOException {
       int id_pessoa = Integer.parseInt(request.getParameter("id_pessoa"));
        try{
            ...
            request.setAttribute("fornecedor", dao.listarFornecedor(id_pessoa));
            request.getRequestDispatcher("alterarfornecedor.jsp").forward(request, response);
        } catch(Exception ex){
            System.out.println("problemas no servlet ao carregar fornecedor!:"+ex.getMessage());
            ex.printStackTrace();
