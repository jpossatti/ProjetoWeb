...
public class CadastrarComodo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String mensagem = comodo.getNome_comodo() + " Cadastrado com Sucesso";

            ComodoDAO dao = new ComodoDAOImp();/*cria o metodo mas ele recebe o AlunoDAOImp*/
            dao.cadastrarComodo(comodo);
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("cadastrarcomodo.jsp")/*Quando for LISTAR usar o servlet no Dispacther*/.forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problema no servlet ao cadastrar comodo! Erro: " + ex.getMessage());
        }
    }
