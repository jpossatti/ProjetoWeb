...
public class ExcluirFornecedor extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        Integer codigo = Integer.parseInt(request.getParameter("id_pessoa"));
        try{
            FornecedorDAO dao= new FornecedorDAOImp();
            dao.excluirFornecedor(codigo);
            request.getRequestDispatcher("ListarFornecedor").forward(request, response);

        }catch (Exception ex){
            System.out.println("Problemas no Servlet as excluir imovel !Erro:"+ex.getMessage());
            ex.printStackTrace();
        }
    }
